...
{else}
    ...
    {assign var=product_price_subtotal_formatted value=$product.product_price_subtotal_excl_formatted*$packageAzai}
{/if}

<tr>
    <td colspan=1 style="border:0px solid #D6D4D4;">
        {if isset($product.image) && $product.image->id}{$packageAzai}<img src="{$product.image_tag|escape:'html':'UTF-8'}" alt="{$product['product_title']|escape:'html':'UTF-8'}" class="img img-thumbnail" />{/if}
    </td>
