...
class CustomerFormatterCore implements FormFormatterInterface
{
    ...
    public function getFormat()
    {
       ...
       // echo $genderField->getName();
        ...
        //$format['seller_code_profile'] = (new FormField)
        $custom_profile_seller = array(
            $this->translator->trans('Tienda', [], 'Shop.Forms.Labels' ), 
            $this->translator->trans('Customer', [], 'Shop.Forms.Labels' ) );
         $customerSellerField = (new FormField)
            ->setName('seller_code_profile')
            ->setType('select')
            ->setLabel(
                $this->translator->trans(
                    'Select client type', [], 'Shop.Forms.Labels'
                )
            )->setRequired(false)
        ;
         
        foreach ($custom_profile_seller as $key=>$custom_seller) {
            $customerSellerField->addAvailableValue($custom_seller);
                if($custom_seller = 0){
                    $custom_seller = 'tienda';
                }else if($custom_seller = 1){
                    $custom_seller = 'customer';
                }
              
        }
        $format[$customerSellerField->getName()] = $customerSellerField;
        //echo $customerSellerField->getName();
        /** Ending customer select**/

        $format['seller_code'] = (new FormField)
            ->setName('seller_code')
            ->setType('text')
            ->setLabel(
                $this->translator->trans(
                    'Seller Code (Optional)', [], 'Shop.Forms.Labels'
                )
            )->addAvailableValue('placeholder', $this->translator->trans(
                    'Enter seller code (optional)', [], 'Shop.Forms.Labels'))
